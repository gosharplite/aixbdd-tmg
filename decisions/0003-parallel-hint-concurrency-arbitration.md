...
- **Related:** [aixbdd-tmg#9](https://github.com/gosharplite/aixbdd-tmg/issues/9) (defect),
  ...
  `skills/axb-implement/rules/ParallelHint平行Subagent與同檔調度判準.md` (Rule 1, Rule 2, Rule 3),
...
2. **Downstream (`axb-implement`) — Disjoint Dispatch vs. Intra-File Serialization / Grouping**:
...
- `skills/axb-tasks/rules/TruthDelta影響盤點與任務型態判準.md`: Added Rule 5 recommending independent landing files
  (Zero Shared Edits, `SHOULD`) for Phase 2 and Phase 3.
- `skills/axb-tasks/SKILL.md`: Mentioned Zero Shared Edits in Phase 2 Foundational and Phase 3 Test Alignment planning.
- `skills/axb-tasks/templates/tasks.md` and `tasks.example.md`: Updated Phase 2 Foundational skeleton setup (T006) and
  Phase 3 tasks (T008–T014) to demonstrate independent landing files per step definition, removing the previous contradiction
  where the worked example modeled the anti-pattern.
- `skills/axb-implement/rules/ParallelHint平行Subagent與衝突Merge判準.md`: Renamed to
  `skills/axb-implement/rules/ParallelHint平行Subagent與同檔調度判準.md` because the thesis changed from in-prompt diff
  merging to concurrency arbitration and intra-file serialization/grouping.
  - Rule 1 updated: Parallel dispatch applies to disjoint files; shared files require serialization or grouping into a single subagent.
...
- `skills/axb-implement/SKILL.md`: Operating Principles and Phase 4 aligned by linking to `ParallelHint平行Subagent與同檔調度判準.md`
  as the single authority rather than inlining paraphrased clauses.
...
- **Domain model**: No change required. The `Task` invariant `task-strict-ordering` (completing and verifying the batch
  before subsequent tasks start) remains fully satisfied. The concept **"Zero Shared Edits"** is an operational executor
  tactic / code organization heuristic between `axb-tasks` and `axb-implement`, not a core domain concept of the product
  development lifecycle; therefore, it does not warrant an entry in `domain-model/aixbdd.modelith.yaml` glossary, keeping
  the domain model lean and focused on the product-truth boundary.
...
| Scenario | Execution Strategy | Expected Outcome |
|---|---|---|
...
| T008 and T009 both write `steps/chat.py` (shared file) | Intra-file Serialization (or grouped into one subagent) | **Compliant** — T008 completes and flushes, then T009 reads latest file; no lost updates. |
| T008 and T009 both write `steps/chat.py` (shared file) | Concurrent Uncoordinated Dispatch | **Violation** — forbidden race condition causing lost updates. |
...
| T015 review task | Audit Only | **Compliant** — checks undefined steps & test compilation without merging diffs or writing code. |
