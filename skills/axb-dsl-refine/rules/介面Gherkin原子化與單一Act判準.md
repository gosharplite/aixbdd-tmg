...
# Rule 2 - 每個 Rule 只能驗證一個受測主體（subject）

- Level: `MUST`
- 每個 Rule 只有一個受測主體：一個具名的業務事件或結果。同一 Rule 的多個 `Then` / `And` 只能是這個主體結果的證據面向，不得因為共用一次操作就把所有後續結果塞進同一案例。
- 判定採「蘊含（entailment）」：先寫出主體結果，再逐一問——在領域模型下，是否存在一個「主體結果成立、但這個斷言不成立」的有效狀態？
  - 不存在（此斷言被主體結果蘊含）→ 可留在同一 Rule。
  - 存在（不被蘊含）→ 這是另一個面向，必須拆成不同 Rule。
- 蘊含只看「有效的領域狀態」，不看「注入人為 bug 後能否單獨失敗」。任何人為 bug 都能讓單一斷言單獨失敗；用「能否單獨失敗」當前題會退化成每個 Rule 恰好一個 `Then`，不具決定性，不得採用（原「只改壞其中一個結果時，這個 Rule 是否仍可能通過」即屬此種失效讀法）。
- 若某個 `Then` 的述詞指向新的主體（另一個可用領域名詞指名的物件或事件），無論是否共用同一次操作，都是第二個面向，必須拆 Rule。
- `STANDARDS.md` §5.2 的 `呈現結果` / `權威狀態` / `再讀確認` / `跨視角` / `不該發生`，加上 exit / verdict 通道，是「同一個主體」的證據通道詞彙；它們不授權把未被蘊含的獨立事實併入同一 Rule。
- 主體結果是否蘊含某個子斷言取決於領域語意；作者必須指出所依據的領域模型、contract 或 data 定義。無法決定時，先把領域語意收斂清楚，不得憑感覺拆或併。
- 下列只是 smell，不是判準：Rule 標題若同時包含成功與失敗、接受與拒絕、呈現結果與狀態不變、歷史寫入與回合切換，通常仍可再拆。

## Calibration Set

- 任何新的原子化判準都必須同時重現下表所有錨點；此表也是 issue #5 的收斂依據。

| 案例 | 受測主體 | 子斷言是否被蘊含 | 判決 |
| --- | --- | --- | --- |
| Rule 2 Good Example：`對局已結束` + `Alice 獲勝` | 一次獲勝事件 | 是（獲勝 ⟹ 對局結束） | 併入同一 Rule |
| CLI 成功建立 workspace：回應內容 + exit 0（tellme#4 Q2） | 一次成功建立 | 是（契約定義下由成功結果決定回應與 verdict 通道） | 併入同一 Rule |
| Rule 1 Good Example：三條三連猜 Rule | 三個互相獨立的結果 | 否 | 拆成三條 Rule |
| Rule 6 Good Example：非當前回合的「拒絕 / 不新增歷史 / 不改變回合」 | 三個可各自失效的禁止 | 否（各 prohibition 可獨立失效） | 拆成三條 Rule |
| tellme#4 W2：`reuses the session workspace` + `workspace 仍持有該檔` | 視 `reuse the workspace` 的領域定義 | 由領域定義決定 | 蘊含則併，否則拆 |
| tellme#4 D2：`configuration resolved` + 各子解析結果 | 視 `resolved` 的領域定義 | 由領域定義決定 | 蘊含則併，否則拆 |

- W2 與 D2 結構同型（容器與其內容物、聚合與其子結果），因此**必須得到相同判決**；若無法從領域定義回答，先補領域語意，不得對其中一個拆、另一個併。
...
- 不可觀察需求（Unobservable NFRs / 內部不變量）若無法表達為外部 Gherkin 步驟，**嚴禁靜默丟棄**，亦不得作為未經見證的散文直接寫入 `dsl.md` 序言或介面註解中；必須明確路由至下游 `/axb-tasks` 作為 `[WITNESS]` 任務或記錄於專案宣告之決策面（ADR）。
...
- 交付前逐一檢查：
  ...
  - 多個 Then / And 是否都被同一受測主體的結果所蘊含（見 Rule 2 的蘊含判定）；未被蘊含者是否已拆成獨立 Rule。
