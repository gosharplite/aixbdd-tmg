...
- **DSL 序言散文的真實邊界**：`dsl.md` 的前言、模組說明或註解文字**嚴禁記載未受見證的系統行為保證或內部不變量**（例如「本模組回滾具耐久性與原子性」、「所有檔案寫入皆保證 fsync 落盤」）。若存在此類保證，可觀察者必須收斂為 Gherkin 步驟與 DSL row；不可觀察者必須由 `[WITNESS]` 單元/故障注入見證任務承接，或由專案宣告之決策面（ADR）記錄為決策，絕不得以未經見證的散文形式留在 `dsl.md` 序言中。
...
- **規範性宣稱路由檢查（Claim Routing）**：不可觀察需求（unobservable NFRs / 內部不變量）不得在轉換為 Gherkin 的過程中被靜默丟棄（silently dropped）。所有源自 `spec.md` 之規範性條目，未進入 Gherkin 者必須顯式路由至 RD 端的 `[WITNESS]` 任務或專案決策面，並納入 `Claim→Witness` 盤點。
