# Rule 1 - 當前 phase 有 Parallel Hint 時必須一次派出該批 subagent（同檔衝突除外）

- Level: `MUST`
- 若 `tasks.md` 當前 phase 含 `Parallel Hint`，本輪任務集就是 Hint 列出的 `[P]` tasks，不是只拿第一個已解鎖 task。
- **目標檔案獨立時全並行**：若該批 tasks 寫入的目標檔案彼此獨立（Zero Shared Edits 原則），每個列出的 `[P]` task 各派一個獨立 subagent，一次派出整批並行執行。
- **同檔衝突時按檔調度**：若多個 `[P]` tasks 寫入同一目標檔案，為避免 Lost Update 與寫入競爭，orchestrator 必須按目標檔案序列排程（同檔任務序列執行，確保前置寫入落盤後後續 subagent 才啟動讀取），或將同檔任務合併由單一 subagent 循序處理；不得在無鎖狀態下讓多個 subagent 同時並行寫入同一實體檔案。
- review task 等該批全部回來後才啟動，不得與該批平行。

## Good Example

- 這個例子是好的，因為獨立檔案一次派出整批，同檔任務則按檔序列排程。

```md
Parallel Hint: T008–T014 各派一個獨立 subagent；T015 等全部回來再 review。
T008–T014 落點為獨立檔案（例如各寫各的 `steps/step_*.go`）：本輪一次並行派出 T008–T014。
若 T008 與 T009 均寫入同一 `操作與斷言.py`：orchestrator 先派 T008，落盤後再派 T009。
```

## Bad Example

- 這個例子是壞的，因為明明落點獨立卻忽略 Hint 依舊全部序列，或明明寫同一檔卻盲目並行導致寫入覆蓋。

```md
T008–T014 寫不同檔案，agent 卻只派一個 subagent 做 T008，T009–T014 留到後面序列。
或者：T008 與 T009 寫同一個檔案，agent 同時並行派出兩個 subagent 改同檔，造成 Lost Update。
```

# Rule 2 - subagent prompt 只指向 tasks.md 的該筆任務

- Level: `MUST`
- 每個 subagent 的 prompt 必須包含：讀哪個 plan 的 `tasks.md`、第幾個任務、職責就是開發此任務。
- 衝突處理寫在 prompt 裡：若為同檔循序任務，執行時先讀取最新落盤內容，再疊加自己這條 DSL 的改動。
- 不得在 `tasks.md` 再貼一份完整 prompt。不得在 prompt 重寫 DSL / Why / Read；那些已在該 task 與 Phase `Shared Must Read`。

## Good Example

- 這個例子是好的，因為 prompt 指向任務本身。

```md
請讀 `specs/plans/004-room-chat-adjustment/tasks.md` 的 T008。
你的職責就是開發此任務。
若目標檔已存在或已被前置任務改動，先讀最新內容，自己加入這條 DSL 的改動。
```

## Bad Example

- 這個例子是壞的，因為它另寫一套語意，不叫 subagent 去讀 task。

```md
請把送訊 stepdef 改成驗證 store。不要去看 tasks.md。
```

# Rule 3 - 寫入責任留在 subagent，禁止同檔無協調並行覆寫，review 不負責合併

- Level: `MUST`
- 禁止多個並行 subagent 同時寫入同一實體檔案（此為 Lost Update 根源）。
- 兩個 Phase 3 task 要寫同一檔時，orchestrator 必須循序調度：前一個 subagent 完成並落盤後，後續 subagent 先讀取最新檔再加入自己這條 DSL 的 stepdef。
- review task 只做品質與覆蓋檢查（確認無 undefined step、測試可正常執行），不負責合併 diff，也不代寫 stepdef。

## Good Example

- 這個例子是好的，因為寫入由 subagent 循序落盤，review 只做檢查。

```md
T008 與 T009 都寫 `操作與斷言.py`。
orchestrator 先派 T008 完成落盤；接著 T009 的 subagent 讀取最新內容，加入自己那條 Then 的 stepdef 並落盤。
T015 只 review 結果，確認沒有 undefined step。
```

## Bad Example

- 這個例子是壞的，因為它讓多個 subagent 同時改同檔造成覆蓋，或者把合併責任丟給 review。

```md
T008 與 T009 同時並行寫入 `操作與斷言.py`，T009 覆蓋 T008 的程式碼。
或者 T008–T011 只交 patch，等 T015 合併。
```

# Rule 4 - review 有 issues 就修正，再 review，直到沒有任何問題

- Level: `MUST`
- Phase 3 的 review task 必須啟動 subagent 去 review。
- 本輪所有 Feature phase 的 `Test Scope` 跑起來不得再有 undefined step；失敗只能是 assertion 或產品行為。
- 只要有 issues 就修正，再啟動 subagent review；重複直到 subagent 沒有任何問題。
- review 通過前，不得解鎖 Feature phase 的 Green。

## Good Example

- 這個例子是好的，因為它形成迴圈閘門。

```md
T015 review 回報 T012 的 stepdef 沒讀 `權威狀態落地`。
修正後再 review。第二次沒有 issues，才解鎖 T016 [BDD-GREEN]。
```

## Bad Example

- 這個例子是壞的，因為它 review 一次有 issues 仍進 Green。

```md
T015 列出 3 個 issues。
agent 說之後 Green 再修，直接開始 T016。
```
