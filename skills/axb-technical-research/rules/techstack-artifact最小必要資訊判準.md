...
# Rule 6 - `techstack.md` 嚴禁將系統行為保證或內部不變量記載為技術項目

- Level: `MUST`
- `techstack.md` 只能記錄技術選型、工具框架、套件依賴與用途分層，**嚴禁**將系統行為保證或內部不變量（如「回滾具耐久性與原子性」、「崩潰不毀損資料」、「不進行外部網路連線」、「重試具備冪等性」等）寫作 `techstack.md` 的表格列或散文說明。
- 行為保證與不變量屬於規格或介面真實（Spec / Interface Truth），不是技術堆疊：
  - 若可由使用者或介面外部觀察：必須導流至 Gherkin 驗收情境與 interface feature。
  - 若屬內部不變量或非可觀察 NFR：必須排定 `[WITNESS]` 任務在單元測試或故障注入層建立鑑別性見證，或在專案決策面（ADR）記錄為決策。
- 嚴禁將未經見證的行為保證偽裝成技術規格留在 `techstack.md`，避免後續工程誤將散文保證視為既有事實。

## Good Example

- 這個例子是好的，因為 `techstack.md` 只保留技術本體，不偷渡行為保證。
...
| 類別 | 採用技術 | 用途 |
| --- | --- | --- |
| 本地儲存 | `os.File` + `syscall.Fsync` | 歷史紀錄檔案寫入與落盤 |
````

## Bad Example

- 這個例子是壞的，因為把未受保護的行為保證當成技術列記載在 `techstack.md`。
...
| 類別 | 採用技術 | 用途 |
| --- | --- | --- |
| 資料持久性 | `原子性回滾機制` | 回滾具原子性與耐久性：temp file + fsync + atomic rename 保證崩潰不損毀 |
````
